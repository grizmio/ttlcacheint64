...
type CheckExpireCallback func(key int64, value interface{}) bool

// ExpireCallback is used as a callback on item expiration or when notifying of an item new to the cache
type ExpireCallback func(key int64, value interface{})
...
type LoaderFunction func(key int64) (data interface{}, ttl time.Duration, err error)
...
type Cache struct {
	...
	loaderLock             map[int64]*sync.Cond
...
func (cache *Cache) getItem(key int64) (*item, bool, bool) {
...
func (cache *Cache) Set(key int64, data interface{}) error {
	return cache.SetWithTTL(key, data, ItemExpireWithGlobalTTL)
}

// SetWithTTL is a thread-safe way to add new items to the map with individual ttl.
func (cache *Cache) SetWithTTL(key int64, data interface{}, ttl time.Duration) error {
	cache.mutex.Lock()
	if cache.isShutDown {
		cache.mutex.Unlock()
		return ErrClosed
...
func (cache *Cache) Get(key int64) (interface{}, error) {
	cache.mutex.Lock()
	if cache.isShutDown {
		cache.mutex.Unlock()
...
func (cache *Cache) invokeLoader(key int64) (dataToReturn interface{}, err error) {
...
func (cache *Cache) Remove(key int64) error {
	cache.mutex.Lock()
...
func (cache *Cache) Purge() error {
	...
	cache.items = make(map[int64]*item)
...
func NewCache() *Cache {
	...
	cache := &Cache{
		...
		loaderLock:             make(map[int64]*sync.Cond),
